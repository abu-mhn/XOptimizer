...
- Deck bonus: a signed-in player who registered a COMPLETE 3-combo deck earns DOUBLE global ranking points at every placing — 1st 5→10, 2nd 4→8, 3rd 3→6, top 8 2→4, played 1→2. A partly-built deck doesn't qualify (same completeness check the host sees in the pre-start warnings), and guests are excluded since they never score ranking points at all. The multiplier is applied after placings resolve, so a doubled participation point (2) can't shadow a genuine top-8 placing (2). Revox points are deliberately untouched, keeping their own 7/5/3/2 scheme so the club leaderboard stays comparable with past events
