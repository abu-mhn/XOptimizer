...
- Pre-Start deck check — the same prompt also sweeps every registrant and lists any deck problems: account registrants with no deck, anyone with incomplete slots (and which slots), and anyone whose deck contains banned parts (and which parts). Guests with no deck are intentionally skipped (deck is optional for guests). Format summary and deck warnings share one dialog, so the host confirms the whole picture at once instead of clearing two prompts back to back — and can still fix the listed issues or override and start anyway
