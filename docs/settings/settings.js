...
- Calling Monitor — a Monitor button opens a big-screen "who's up" board for a projector / TV (a separate window on desktop, a full-screen overlay on mobile). NOW CALLING shows the matches you've made LIVE (large player names + group / round or knockout label), UP NEXT lists the next unplayed matches, with a live clock and the tournament name; it updates live off the room and opens maximized (a corner button or F11 goes true fullscreen). Each live match shows its running score as it's played and a "Called by <judge>" line naming whoever went live on it, so players know who's calling them. Built-in voice call-outs announce each match the moment it goes LIVE ("Now calling: A versus B, Group A") in a female voice at a natural pace — on automatically, no setup
