...
- Match-start alerts — when a host goes LIVE on a match, the other devices connected to the room pop an in-app toast in the top-right naming the players + round / group, with a "You're up!" green highlight when the signed-in user's username is one of the players. Tap to dismiss or auto-dismisses after 8s. Notifications are role-gated: host AND co-hosts get every match, participants only get notifications for their OWN matches, viewers stay silent. Optional system notifications via the "Turn on alerts" button in the first toast — once Notification permission is granted, every later match-start also fires an OS-level notification through the site's service worker so users still get pinged when the phone is locked or the browser tab is in the background (mobile Chrome / Edge / Brave supported via the SW path; iOS Safari needs the site installed as a PWA via Add to Home Screen). A Match Alerts row in Settings shows the current state (unsupported / blocked / off / on) and lets you turn alerts on, or mute them per device without revoking the permission
