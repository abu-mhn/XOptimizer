...
- Calling Monitor (desktop) — a Monitor button opens a big-screen "who's up" board in a separate window for a projector / TV. NOW CALLING shows the matches you've made LIVE (large player names + group / round), UP NEXT lists the next unplayed matches, with a live clock and the tournament name; it updates live off the room and opens maximized (a corner button or F11 goes true fullscreen). Built-in voice call-outs announce each match the moment it goes LIVE ("Now calling: A versus B, Group A") in a female voice at a natural pace — on automatically, no setup
